# # Make scatter plot with points showing computed CES score for each HOLC 
# # neighborhood, with scores computed using the area weighted average method
# # on the x-axis, and scores computed using the nearest centroid method on the 
# # y-axis. 
# 
# # packages -----------------------------------------------------------
# library(tidyverse)
...
f_plot_scores_method_comparison_scatter <- function(df_holc_ces_scores_comparison,
    ...
    ## make plot ----
    method_comarison_scatter <- ggplot(data = df_holc_ces_scores_comparison,
    ...
    ggsave(filename = here(output_directory, 
           ...
           plot = method_comarison_scatter, 
           # bg = 'transparent',
           bg = 'white',
           scale = 1, #1.5 # increasing scale reduces relative size of text
           width = 6, 
           height = 6, # 4.5
           dpi = 300)
    ...
    return(method_comarison_scatter)
