...
{
    geojson_fr <- (readLines(url_fresno) %>% 
                       fromJSON())$features
...
## Convert geojson to dataframe
{
    geojson_sd <- (readLines(url_sandiego) %>% 
                       fromJSON())$features
...
## Convert geojson to dataframe
{
    geojson_sf <- (readLines(url_sanfrancisco) %>% 
                       fromJSON())$features
...
## Convert geojson to dataframe
{
    ...
    geojson_la <- (readLines(url_losangeles) %>% 
                       fromJSON())$features
...
## Convert geojson to dataframe
{
    geojson_sac <- (readLines(url_sacramento) %>% 
                        fromJSON())$features
...
## Convert geojson to dataframe
{
    geojson_stk <- (readLines(url_stockton) %>% 
                        fromJSON())$features
...
## Convert geojson to dataframe
{
    geojson_oak <- (readLines(url_oakland) %>% 
                        fromJSON())$features
...
## Convert geojson to dataframe
{
    geojson_sj <- (readLines(url_sanjose) %>% 
                       fromJSON())$features
...
# create a new column with area description excerpts (for use in the app in the popup for each holc polygon)
